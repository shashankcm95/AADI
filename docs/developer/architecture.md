...
Arrive is a GPS-powered just-in-time kitchen orchestration platform. Its architecture is serverless, event-driven, and designed for independent service deployment. This document explains how the system is structured, why it is structured that way, and how a request flows from the client to the database and back.
