...
**Geofence signal loss** on mobile devices can cause missed presence detection. If a customer's device loses GPS signal inside the restaurant (common in dense buildings) or the app is killed by the OS, the geofence presence confirmation may not trigger automatically. The system mitigates this with manual vicinity reporting (the customer can tap a button in the app) and the same-location bootstrap that detects the customer is already at the restaurant, but the automatic geofence path has an inherent dependency on device GPS availability.
